...
const RadioButton=({label,selected=false,onChange=()=>{}})=>{
    return (
        <label className='flex gap-3 items-center cursor-pointer mt-2 text-sm'>
            <input type='radio' name='sortOptions' checked={selected} onChange={()=>onChange(label)} className='w-4 h-4 accent-blue-500'/>
            <span className='font-light select-none'>{label}</span>
        </label>
    )
}
...
const AllRooms = () => {
    ...
    const [openFilters, setOpenFilters] = useState(false);
     ...
     const amenities = ['Free Wi-Fi', 'Breakfast Included', 'Parking', 'Pool', 'Gym', 'Spa'];
    ...
    <div  className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32'>
       <div>
        <div className='flex flex-col items-start text-left'> 
            ...
            <p className='text-sm md:text-base text-gray-500/90 mt-2 max-w-174'>Take advantage of our limited-time offers and special packages to enhance your stay and cretae unforgettable memeories.</p>
        </div>

       {roomsDummyData.map((room)=>(
        <div className='flex flex-col md:flex-row items-start py-10  border-b border-gray-300  gap-6  last:pb-30 last:border-0' key={room._id}>
